...
def project_team_runs(lineup_matchups: list[dict], park_factor: float, weather_mult: float) -> TeamProjection:
    """`lineup_matchups` is a list of dicts in batting-order sequence, each
    with keys: name, matchup_wOBA, and blended batter rates for props.
    """
    total_runs = 0.0
    enriched: list[dict] = []
    ...
    total_runs *= park_factor * weather_mult
    return TeamProjection(team=lineup_matchups[0].get("_team", ""), expected_runs=total_runs, lineup=enriched)
...
def batter_props(row: dict) -> dict:
    """Convert a batter's blended rates + matchup wOBA into simple projections.

    We ground the projections in the batter's blended AVG/SLG/HR-rate when
    provided, then scale by the matchup's multiplier vs league wOBA.
    """
    pa = row["PA"]
    ...
    exp_ab = pa * 0.90  # BB/HBP/SF removed, rough ratio
    exp_hits = (avg * exp_ab * mult) if avg is not None else None
    exp_tb = (slg * exp_ab * mult) if slg is not None else None
    exp_hr = (hr_rate * pa * mult) if hr_rate is not None else None
    return {
        "exp_PA": round(pa, 2),
